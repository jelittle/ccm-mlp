...
def get_metadata(image_path):
    ...
    metadata['calibration_illuminant_1'], metadata['calibration_illuminant_2'] = get_calibration_illuminant(tags, ifds)


    #active area cropping is tag 50829
    # ...
...
def get_hsv_luts(tags, ifds):
    ...
    if (hue_sat_map_dims is not None and hsv_lut_1 is not None):
        ...
        hsv_lut = np.reshape(hsv_lut_1, hue_sat_map_dims)
...
def get_profile_luts(tags, ifds):
    ...
    if (profile_dims is not None and profile_lut is not None):
        ...
        profile_lut = np.reshape(profile_lut, profile_dims)
...
def get_values(tags, possible_keys):
    ...
    return values
def get_calibration_illuminant(tags, ifds):
    possible_keys_1 = ['Image Tag 0xC65A', 'Image Tag 50778', 'CalibrationIlluminant1', 'Image CalibrationIlluminant1']
    calibration_illuminant_1 = get_values(tags, possible_keys_1)
    possible_keys_2 = ['Image Tag 0xC65B', 'Image Tag 50779', 'CalibrationIlluminant2', 'Image CalibrationIlluminant2']
    calibration_illuminant_2 = get_values(tags, possible_keys_2)
    return calibration_illuminant_1, calibration_illuminant_2
